...
func (pq *PostQuery) QueryPhoto() *ImageQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2O, true, post.PhotoTable, post.PhotoColumn),
		)
		fromU = sqlgraph.SetNeighbors(pq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (pq *PostQuery) WithPhoto(opts ...func(*ImageQuery)) *PostQuery {
	query := &ImageQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
...
func (pq *PostQuery) sqlAll(ctx context.Context) ([]*Post, error) {
	...
	if pq.withUser != nil || pq.withPhoto != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := pq.withPhoto; query != nil {
		ids := make([]int, 0, len(nodes))
		nodeids := make(map[int][]*Post)
		for i := range nodes {
			if nodes[i].image_posts == nil {
				continue
			}
			fk := *nodes[i].image_posts
			if _, ok := nodeids[fk]; !ok {
				ids = append(ids, fk)
			}
			nodeids[fk] = append(nodeids[fk], nodes[i])
		...
		for _, n := range neighbors {
			nodes, ok := nodeids[n.ID]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "image_posts" returned %v`, n.ID)
			}
			for i := range nodes {
				nodes[i].Edges.Photo = n
			}
		}
	}
