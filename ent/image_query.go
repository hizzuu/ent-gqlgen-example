...
func (iq *ImageQuery) QueryPosts() *PostQuery {
	query := &PostQuery{config: iq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2O, false, image.PostsTable, image.PostsColumn),
		)
...
func (iq *ImageQuery) QueryUsers() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2O, false, image.UsersTable, image.UsersColumn),
		)
		fromU = sqlgraph.SetNeighbors(iq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (iq *ImageQuery) WithPosts(opts ...func(*PostQuery)) *ImageQuery {
	query := &PostQuery{config: iq.config}
...
func (iq *ImageQuery) sqlAll(ctx context.Context) ([]*Image, error) {
	...
	if query := iq.withPosts; query != nil {
		...
		query.Where(predicate.Post(func(s *sql.Selector) {
			s.Where(sql.InValues(image.PostsColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.image_posts
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "image_posts" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "image_posts" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Posts = n
		}
	}
	...
	if query := iq.withUsers; query != nil {
		...
		for _, n := range neighbors {
			fk := n.image_users
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "image_users" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "image_users" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Users = n
		}
	}
